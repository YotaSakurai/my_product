...
void high_isr (void)
{
	if(INTCON3bits.INT2IF == 1){		//割り込みフラグ
...
void low_isr (void)
{	
	if(INTCON3bits.INT1IF == 1){		//INT1割り込み 
		INTCON3bits.INT1IF = 0;			//INT1割り込みクリア
		if(PORTBbits.RB0 == 1){			//B相出力状態確認			////A相?
			A--;						//カウント値－
		}
		else{
			A++;						//カウント値+
...
//		if(timer1 > 0x15B || timer1 < 0x5A ){							//カムでの角度が347.4°(信号立下り)								
		if(timer1 > 683 || timer1 < 180 ){	
			LATAbits.LATA1 = 1;					//BTDC18° 燃料噴射
		}
		else{
			LATAbits.LATA1 = 0;
		}	
		...
		if(NoRev == 0){							//NoRev =0なら
			...
			Rev1 = Rev;							//1回前の回転数
			doel=(unsigned int)705-(337500/Zcount);
			doel2=doel;
...
{
	lcd_cmd(0x80);							//１行目へ
//	ltostring(3,(18*A)/100, &Msg1[0]+0);	//バルブタイミング調整
//	ltostring(2,(18*A)%100, &Msg1[0]+4);	//バルブタイミング調整
//	ltostring(4,Ave, &Msg1[0]+0);			//平均回転数
	sprintf(Msg1,"%3d.%2d",(5*A)/10,(5*A)%10);		////文字列書式入出力関数
	lcd_str((char *)Msg1);					//1行目表示
 
 	lcd_cmd(0xC0);							//2行目へ					359.5
//	ltostring(4,CCPR1,&Msg2[0]+0);			//コイル通電 ON
//	ltostring(4,CCPR2,&Msg2[0]+4);			//コイル通電　OFF
//	ltostring(4,timing, &Msg2[0]+0);		//パルス確認用
//	ltostring(4,Rev, &Msg2[0]+0);			//平均回転数
